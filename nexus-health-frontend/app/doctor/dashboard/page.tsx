...
export default function DoctorDashboard() {
  ...
  return (
    <div>
      ...
      <Card className="bg-slate-900/50 border-slate-800"><CardHeader><CardTitle className="text-white">Today&apos;s Appointments</CardTitle></CardHeader>
        <CardContent>{today.length===0?<p className="text-slate-500 py-6 text-center">None today</p>:<div className="space-y-3">{today.map(apt=>(<div key={apt.id} className="flex items-center justify-between p-4 rounded-lg bg-slate-800/30 border border-slate-800"><div><p className="font-medium text-white">{apt.reason}</p><p className="text-sm text-slate-400">{apt.timeSlot}</p></div><span className="text-xs px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-400">{apt.status}</span></div>))}</div>}</CardContent>
      </Card>
    </div>
  );
}
